...
class Word2Vec_Skipgram_Data(object):
    ...
    def build_graph(self):
        ...
        (words, counts, words_per_epoch, self._epoch, self._words, examples, labels) \
            = word2vec.skipgram_word2vec(filename=self.train_data,
                                         batch_size=self.batch_size,
                                         window_size=self.window_size,
                                         min_count=self.min_count,
                                         subsample=self.subsample)
        (self.vocab_words, self.vocab_counts, self.words_per_epoch) = self._session.run([words, counts, words_per_epoch])
