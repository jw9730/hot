...
from .model import EncoderS2G, MLPS2G
...
class SparseEmbedding(nn.Module):
    def __init__(self, embedding_weight, sparse=True):
        ...
        if self.sparse:
            ...
            try:
                try:
                    self.embedding = torch.from_numpy(np.asarray(embedding_weight.todense())).to(device)
                except BaseException:
                    self.embedding = torch.from_numpy(np.asarray(embedding_weight)).to(device)
...
class MultipleEmbedding(nn.Module):
    def __init__(self, embedding_weights, dim, sparse=True, num_list=None, node_type_mapping=None):
        super().__init__()
        print(dim)
...
class Word2vec_Skipgram(nn.Module):
    def __init__(self, dict_size, embedding_dim, window_size, u_embedding=None, sparse=False):
        ...
        self.sm_w_t = nn.Embedding(dict_size, embedding_dim, sparse=sparse, padding_idx=0)
...
class Classifier(nn.Module):
    def __init__(self, n_head, d_model, d_k, d_v, node_embedding, diag_mask, bottle_neck, num_list,
                 set2graph, use_transformer,
                 n_phi_layers, n_psi_layers, dropout_phi, dropout_psi, simple_mlp):
        super().__init__()
        self.node_embedding = node_embedding

        self.set2graph = set2graph
        if self.set2graph:
            self.num_list = num_list
            phi_hidden_dims = [d_model] * (n_phi_layers - 1)
            psi_hidden_dims = [d_model] * (n_psi_layers - 1)
            if use_transformer:
                self.net = EncoderS2G(d_model, 1, phi_hidden_dims, d_k * n_head, d_k * n_head, d_model, n_head,
                                      dropout_phi, dropout_psi, psi_hidden_dims, simple_mlp)
            else:
                self.net = MLPS2G(d_model, 1, phi_hidden_dims, dropout_phi, dropout_psi, psi_hidden_dims)
        else:
    ...
    def forward_default(self, x, mask=None, get_outlier=None, return_recon=False):
        x = x.long()
        slf_attn_mask = get_attn_key_pad_mask(seq_k=x, seq_q=x)
        non_pad_mask = get_non_pad_mask(x)
        if return_recon:
        ...
        mode = 'sum'
        if mode == 'min':
            output, _ = torch.max((1 - output) * non_pad_mask, dim=-2, keepdim=False)
            output = 1 - output
        elif mode == 'sum':
    ...
    def forward_set2graph(self, indices, mask=None, get_outlier=None, return_recon=False):
        assert get_outlier is None and mask is None
        indices = indices.long()
        non_pad_mask = get_non_pad_mask(indices)

        ids = torch.Tensor(np.arange(int(self.num_list[-1])) + 1).long().to(device).view(-1, 1)
        if return_recon:
            embeddings, recon_loss = self.get_node_embeddings(ids, return_recon=True)
        else:
            embeddings = self.get_node_embeddings(ids, return_recon=False)
        embeddings = embeddings[:, 0, :]

        input_indices = (indices - 1).clamp_(min=0)
        output = self.net(embeddings, input_indices).unsqueeze(1)
        output = torch.sigmoid(output)
    ...
    def forward(self, x, mask=None, get_outlier=None, return_recon=False):
        if self.set2graph:
            return self.forward_set2graph(x, mask, get_outlier, return_recon)
        else:
            return self.forward_default(x, mask, get_outlier, return_recon)
...
class PositionwiseFeedForward(nn.Module):
    def __init__(self, dims, dropout=None, reshape=False, use_bias=True, residual=False, layer_norm=False):
...
class ScaledDotProductAttention(nn.Module):
    ...
    def masked_softmax(vector: torch.Tensor, mask: torch.Tensor, dim: int = -1,
                       memory_efficient: bool = False, mask_fill_value: float = -1e32) -> torch.Tensor:
        if mask is None:
    ...
    def forward(self, q, k, v, diag_mask, mask=None):
        ...
        attn = self.masked_softmax(attn, diag_mask, dim=-1, memory_efficient=True)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, diag_mask, mask=None):
        ...
        static = static.permute(1, 2, 0, 3).contiguous().view(sz_b, len_q, -1)  # b x lq x (n*dv)
